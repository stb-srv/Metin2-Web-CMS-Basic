...
class CmsController {
    ...
    async getSettings(req, res) {
        let settings = cache.get('site_settings');
        if (!settings) {
    ...
    async updateSettings(req, res) {
        const {
            ...
            theme_color_text_main_dark,
            theme_color_text_muted_dark,
            theme_color_text_accent_dark,
            theme_color_menu_header_dark,
            theme_color_text_main_light,
            theme_color_text_muted_light,
            theme_color_text_accent_light,
            theme_color_menu_header_light,
            theme_bg_color_dark,
        ...
        const settings = {
            ...
            // Dark Mode Variables
            theme_color_text_main_dark: reset_styling === 'true' ? '' : (theme_color_text_main_dark || ''),
            theme_color_text_muted_dark: reset_styling === 'true' ? '' : (theme_color_text_muted_dark || ''),
            theme_color_text_accent_dark: reset_styling === 'true' ? '' : (theme_color_text_accent_dark || ''),
            theme_color_menu_header_dark: reset_styling === 'true' ? '' : (theme_color_menu_header_dark || ''),
            theme_bg_color_dark: reset_styling === 'true' ? '' : (theme_bg_color_dark || ''),

            // Light Mode Variables
            theme_color_text_main_light: reset_styling === 'true' ? '' : (theme_color_text_main_light || ''),
            theme_color_text_muted_light: reset_styling === 'true' ? '' : (theme_color_text_muted_light || ''),
            theme_color_text_accent_light: reset_styling === 'true' ? '' : (theme_color_text_accent_light || ''),
            theme_color_menu_header_light: reset_styling === 'true' ? '' : (theme_color_menu_header_light || ''),
            theme_bg_color_light: reset_styling === 'true' ? '' : (theme_bg_color_light || '')
        };
        
        // Ensure legacy fields are also cleared on reset
        if (reset_styling === 'true') {
            settings.theme_primary_color = '';
            settings.theme_accent_color = '';
            settings.theme_bg_color = '';
            settings.theme_border_radius = '';
            settings.theme_bg_image = '';
            settings.theme_color_text_main = '';
            settings.theme_color_text_muted = '';
            settings.theme_color_text_accent = '';
            settings.theme_color_menu_header = '';
        }

        await repository.updateSettings(settings);
        cache.invalidate('site_settings'); // Force refresh cache on update
        cache.clearAll(); // Best to clear all since settings affect everything (theme, modules, etc)
    ...
    async getNavbarLinks(req, res) {
        let links = cache.get('navbar_links');
        if (!links) {
            links = await repository.getNavbarLinks();
            cache.set('navbar_links', links, 3600);
        }
        res.json({ success: true, links });
    }

    async createNavbarLink(req, res) {
        await repository.createNavbarLink(req.body);
        ...
        await auditLogger.log({
            ...
            target_type: 'navbar',
            target_id: req.body.label,
            ip: req.ip
        });
        cache.invalidate('navbar_links');
        res.json({ success: true, message: 'Link hinzugefügt.' });
    }

    async updateNavbarLink(req, res) {
        await repository.updateNavbarLink(req.params.id, req.body);
        cache.invalidate('navbar_links');
        res.json({ success: true, message: 'Link aktualisiert.' });
    }

    async reorderNavbarLinks(req, res) {
        const { links } = req.body; // Expects array of { id, display_order }
        if (!links || !Array.isArray(links)) {
            return res.status(400).json({ success: false, message: 'Keine Links zum Sortieren empfangen.' });
        }
        await repository.reorderNavbarLinks(links);
        cache.invalidate('navbar_links');
        res.json({ success: true, message: 'Reihenfolge gespeichert.' });
    }
    ...
    async getActiveTheme(req, res) {
        let theme = cache.get('active_theme');
        if (!theme) {
            theme = await repository.getActiveTheme();
            cache.set('active_theme', theme, 3600);
        }
