...
async function isSuperAdmin(req, res, next) {
    ...
    try {
        ...
        // 1. Check RBAC SuperAdmin
        const [rbacRows] = await db.query(`
            SELECT r.name 
            FROM ${s('website')}.account_roles ar
            JOIN ${s('website')}.roles r ON ar.role_id = r.id
            WHERE ar.account_id = ? AND r.name = "SuperAdmin"
        `, [accountId]);

        // 2. Check Legacy IMPLEMENTOR
        const [gmRows] = await db.query(`SELECT mAuthority FROM ${s('common')}.gmlist WHERE mAccount = ? AND mAuthority = "IMPLEMENTOR"`, [loginName]);

        if (rbacRows.length === 0 && gmRows.length === 0) {
            return res.status(403).json({ success: false, message: 'Zugriff verweigert! Du benötigst die Rolle "SuperAdmin" oder "IMPLEMENTOR".' });
        }
