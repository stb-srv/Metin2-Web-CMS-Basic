...
const DashboardPage = {
    ...
    render() {
        return `
            ...
            <div class="admin-grid admin-grid-2">
                ...
                <div class="admin-card">
                    <div class="admin-card-header">
                        <div class="admin-card-title"><i class="fas fa-server"></i> System Status</div>
                    </div>
                    <div style="display: flex; align-items: center; justify-content: space-between; padding: 0.5rem 0;">
                        <div>
                            <div style="font-weight: 600; margin-bottom: 4px;">Wartungsmodus</div>
                            <div style="font-size: 0.8rem; color: var(--text-muted);">Spieler können sich nicht einloggen.</div>
                        </div>
                        <div>
                            <button id="maintenanceToggleBtn" class="btn-admin btn-secondary" onclick="DashboardPage.toggleMaintenance()">
    ...
    async loadStats() {
        try {
            ...
            if (data.success) {
                ...
                this.isMaintenance = data.stats.maintenance_mode;
    ...
    updateMaintenanceUI() {
        ...
        if (this.isMaintenance) {
            btn.className = 'btn-admin btn-danger';
            btn.innerHTML = '<i class="fas fa-ban"></i> Wartung ist AKTIV';
        } else {
            btn.className = 'btn-admin btn-secondary';
            btn.innerHTML = '<i class="fas fa-play"></i> Wartung INAKTIV';
        }
    },

    async toggleMaintenance() {
        if (!await customConfirm(
            this.isMaintenance ? 'Wartungsmodus beenden?' : 'Wartungsmodus wirklich aktivieren?', 
            this.isMaintenance ? 'Der Server wird wieder für alle Spieler freigegeben.' : 'Nur noch Accounts mit Team-Rechten können sich einloggen!'
        )) return;

        try {
            const data = await apiPost('/admin/core/toggle-maintenance');
            if (data.success) {
                this.isMaintenance = data.maintenance_mode;
                this.updateMaintenanceUI();
                showToast(data.message || 'Status geändert', 'success');
            }
        } catch (err) {
