...
const CmsNavbarPage = {
                ...
                <div class="admin-card">
                    <div class="admin-card-header">
                        ...
                        <div style="display:flex; gap:10px;">
                            <button id="saveNavOrderBtn" class="btn-admin btn-success btn-sm" style="display:none;" onclick="CmsNavbarPage.saveOrder()">
                                <i class="fas fa-save"></i> Sortierung speichern
                            </button>
    ...
    initSortable() {
        const el = document.getElementById('navLinkList');
        ...
        Sortable.create(el, {
            ...
            onEnd: () => {
                const btn = document.getElementById('saveNavOrderBtn');
    ...
    async saveOrder() {
        const el = document.getElementById('navLinkList');
        const btn = document.getElementById('saveNavOrderBtn');
        const items = el.querySelectorAll('.nav-link-item');
        
        const reorder = Array.from(items).map((item, index) => ({
            id: parseInt(item.getAttribute('data-id')),
            display_order: index + 1
        }));

        try {
            if (btn) btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Speichere...';
            const res = await apiFetch('/cms/navbar/reorder', {
                method: 'POST',
                body: JSON.stringify({ links: reorder })
            });
            if (res.success) {
            ...
            if (btn) btn.innerHTML = '<i class="fas fa-save"></i> Sortierung speichern';
        }
    },

    renderNavLinkItem(link, isChild = false) {
        return `
            <div class="nav-link-item" data-id="${link.id}" style="display: flex; align-items: center; gap: 10px; background: rgba(255,255,255,0.03); padding: 10px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.05); margin-left: ${isChild ? '30px' : '0'}; margin-top: 5px;">
