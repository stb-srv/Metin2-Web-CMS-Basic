...
window.initAdminNotifications = function() {
    if (adminSSE) return; // Already running

    // EventSource doesn't support custom headers easily, so we rely on cookies
    adminSSE = new EventSource('/api/admin/notifications/stream');

    adminSSE.addEventListener('payment_request', function(e) {
        try {
            const data = JSON.stringify(JSON.parse(e.data)); // Re-parse safely
            ...
            if (typeof Swal !== 'undefined') {
                Swal.fire({
                    ...
                    html: `<b>${parsed.username || 'Jemand'}</b> möchte spenden/aufladen.<br>Betrag: <b>${parsed.amount}€</b> (${parsed.method})`,
                    ...
                    didOpen: (toast) => {
                        ...
                        toast.onclick = () => window.location.href = '/admin/#payments';
                    }
                });
            } else {
                console.log('[Admin Notification]', parsed);
            }
        } catch (err) {
    ...
    adminSSE.onerror = function(err) {
        // If 401/403 or network issue, close it to prevent spamming
        console.warn('Admin SSE connection error/disconnected.');
        adminSSE.close();
        adminSSE = null;
    };
};
