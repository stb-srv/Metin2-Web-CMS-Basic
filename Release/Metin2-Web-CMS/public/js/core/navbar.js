...
window.getFlag = function(lang) {
    const flags = { 
        de: 'de', en: 'us', it: 'it', es: 'es', 
        pl: 'pl', ro: 'ro', tr: 'tr', pt: 'pt' 
    };
...
/**
 * Updates the language switcher UI (flag and label) without full re-render
 */
function updateLanguageSwitcher() {
    const selector = document.querySelector('.language-selector');
    if (!selector || !window.i18n) return;
    
    const trigger = selector.querySelector('a');
    if (!trigger) return;
    ...
    if (flagImg) flagImg.src = window.getFlag ? window.getFlag(currentLang) : `https://flagcdn.com/w20/us.png`;
    if (labelSpan) labelSpan.textContent = currentLang.toUpperCase();
}
...
async function renderNavbarLinks() {
    ...
    try {
        const data = await window.apiFetch('/cms/navbar');
        isNavbarRendering = false;
        
        if (data.success) {
            const parents = data.links.filter(l => !l.parent_id && l.is_active);
            linkContainer.innerHTML = parents.map(parent => {
                const pKey = parent.label || parent.title_key || '';
                const title = window.i18n.t(pKey, pKey);
                const children = data.links.filter(l => l.parent_id === parent.id && l.is_active);
                
                if (children.length > 0) {
                    let pUrl = parent.url;
                    if (pUrl !== '#' && !pUrl.startsWith('javascript:')) {
                        pUrl = pUrl.startsWith('http') || pUrl.startsWith('/') ? pUrl : '/' + pUrl;
                    }
                    return `
                    <div class="nav-dropdown">
                        <a href="${pUrl}" ${pUrl === '#' || pUrl.startsWith('javascript:') ? 'onclick="event.preventDefault();"' : ''} style="cursor: pointer;"><i class="${parent.icon}"></i> ${title}</a>
                        <div class="nav-dropdown-content">
                            ${children.map(child => {
                                let cUrl = child.url;
                                if (cUrl !== '#' && !cUrl.startsWith('javascript:')) {
                                    cUrl = cUrl.startsWith('http') || cUrl.startsWith('/') ? cUrl : '/' + cUrl;
                                }
                                const cKey = child.label || child.title_key || '';
                                const cTitle = window.i18n.t(cKey, cKey);
