...
router.post('/test-db', async (req, res) => {
    ...
    try {
        ...
        const s = (name) => process.env[`DB_SCHEMA_${name.toUpperCase()}`] || name;
        ...
        const dbStatus = {
            account: dbNames.includes(s('account')),
            player: dbNames.includes(s('player')),
            common: dbNames.includes(s('common')),
            website: dbNames.includes(s('website'))
        };
...
router.post('/save-config', (req, res) => {
    ...
    try {
        ...
        // Neu laden der Umgebungsvariablen für laufenden Prozess
        require('dotenv').config({ path: ENV_PATH, override: true });
        
        // DB-Verbindung mit neuen Daten rekreieren
        const db = require('../../config/database');
        if (db.recreatePool) db.recreatePool();
