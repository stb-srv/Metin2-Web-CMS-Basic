...
class CmsRepository {
    ...
    async getNavbarLinks() {
        const { s } = db;
        const [rows] = await db.query(`SELECT * FROM ${s('website')}.navbar_links ORDER BY display_order ASC`);
        return rows;
    }

    async updateNavbarLinks(links) {
        const { s } = db;
        const connection = await db.getConnection();
        try {
            await connection.beginTransaction();
            // Delete existing
            await connection.query(`DELETE FROM ${s('website')}.navbar_links`);
            // Re-insert
            for (const link of links) {
                const { title_key, url, icon, is_external, parent_id, display_order, is_active } = link;
                await connection.query(`
                    INSERT INTO ${s('website')}.navbar_links (title_key, url, icon, is_external, parent_id, display_order, is_active)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                `, [title_key, url, icon, is_external || 0, parent_id || null, display_order || 0, is_active !== undefined ? is_active : 1]);
            }
            await connection.commit();
        } catch (err) {
            await connection.rollback();
